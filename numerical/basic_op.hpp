...
template<typename U>
void remove_channel(U& label_prob,const shape<3>& image_dim,size_t bg_channel = 0)
{
    size_t s = image_dim.size();
    std::copy(label_prob.begin() + (bg_channel+1)*s,label_prob.end(),label_prob.begin() + bg_channel*s);
